...
MDAM = function(sub_dat, niter = 10000, burnin = 5000, pop_X1_mean = sum(population$X1), pop_X2_mean = sum(population$X2)){
  ...
  for (i in 1:niter){
    ...
    ###### Impute unit nonresponse for X1  ######
    # IM6-IM7
  ...
  # Create L=50 multiple imputation datasets
...
RESULTS = foreach(i = 1:iterations, .options.snow = opts) %dopar% {
  MDAM(sub_dat=SAMPLE[[i]])
}
close(pb)
stopCluster(cl)
saveRDS(RESULTS,file = "MI_MH.rds")
